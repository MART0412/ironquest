...
- **Current phase: 3 — Disciplines + measurement (reordered 2026-07-28).** Finish the *fitness* experience before introducing a second life-change; nutrition intelligence is a separate behaviour change and adoption is easier one at a time. Slices in order:
  1. **Disciplines / multiclassing** (B3) — **structure ✅ (2026-08-17)**: `disciplines` (with `logging_style` sets|endurance|session) + `user_disciplines`; `exercises.discipline_id` and `skill_paths.discipline_id` backfilled to calisthenics; `activate_discipline` is the sole writer of `user_disciplines` and enforces the gate (first discipline free, a second needs level ≥ `MULTICLASS_MIN_LEVEL` = 15 in `lib/game/disciplines.ts`, mirrored by `xp_for_level` in SQL). Onboarding gained a discipline step, /profile a Disciplines card; /skills groups by discipline (heading only when >1 active); home's quest and the routine builder scope to activated disciplines. **Still open:** the four non-calisthenics libraries + their skill paths (flip `hasLibrary` in `DISCIPLINE_META` when one lands), endurance/session logging flows, discipline-flavoured stat radar. Schema note: `exercises.branch` is untouched — `discipline_id` sits alongside it.
  2. **Endurance logging** — **✅ (2026-08-27)**: `workouts.kind` (sets|endurance|activity) plus `activity_slug`/`duration_min`/`distance_km`/`notes`, so a run *is* a completed workout row and the streak gap-fill, lifetime session counter and "done today" all keep working unchanged. `log_activity` is the sole writer: XP = duration × MET × `XP_PER_MET_MINUTE`, trimmed to a `DAILY_CAP` of 150/day under the `activity_session` ledger action, and a session ≥ `STREAK_MIN_MINUTES` (10) counts as a streak day on the same rules as a workout. **`lib/fitness/activities.ts` is the source of truth** for MET values and is shared on purpose — Phase 4's food-compensation feature consumes `kcalBurned` from it; the `activities` table mirrors the METs because rule 6 puts the XP decision in SQL, and the live E2E asserts they agree. The flat `bonus_workout` award is untouched. Manual entry only — **GPS/wearable import is a future integration**, deliberately not scaffolded.
  3. **Running & cycling progressions** — **✅ (2026-08-28)**: six ladders / 37 nodes seeded from `lib/game/endurance-paths.ts` (the authoring surface; the live check asserts the DB matches). Three new `unlock_criteria` kinds — `distance` and `pace` judged on a SINGLE session, `frequency` on a rolling window — mirrored by `meetsEnduranceCriteria` in `lib/game/skills.ts` and `endurance_criteria_met` in SQL. `log_activity` awards them: **cascade only where rungs nest.** Distance and pace contain each other (a marathon contains a five), so lower rungs are credited at 25%; a frequency window contains nothing, so only rungs actually met are paid, at full price. One full-price award **per path**, not per session — the ladders are independent. Endurance nodes have no challenge offers or attempt panel: the node sheet links to /activity, because a run either clears the distance or it doesn't. `exercises.branch` now also admits `distance|pace|consistency`; `movement_family` stays null so runs never touch the lifetime rep counters. The routine builder filters to `logging_style='sets'` so endurance nodes can't be added to a routine. Per-discipline stat radars live in `DISCIPLINE_RADARS` (`lib/game/stats.ts`): calisthenics keeps its pentagon, running gets DISTANCE/PACE/CONSISTENCY, cycling DISTANCE/SPEED/CONSISTENCY.
  4. **Body evolution by training style** (B2-evolution) — depends on 1 defining the disciplines.
  5. **Weekly check-in flow** (weight + measurements + photos → `checkins`, Storage bucket + trend charts). Stays in near-term scope: it *measures the fitness goal*, it is not nutrition.
