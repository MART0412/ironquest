...
- **Current phase: 3 — Disciplines + measurement (reordered 2026-07-28).** Finish the *fitness* experience before introducing a second life-change; nutrition intelligence is a separate behaviour change and adoption is easier one at a time. Slices in order:
  ...
  3. **Body evolution by training style** (B2-evolution) — depends on 1 defining the disciplines.
  4. **Weekly check-in flow** (weight + measurements + photos → `checkins`, Storage bucket + trend charts). Stays in near-term scope: it *measures the fitness goal*, it is not nutrition.
